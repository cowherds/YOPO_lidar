...
class LatticeParam:
    def __init__(self):
        ...
        print(f"| {'sensor':<12} = {'LiDAR 360':>6} |")
        print(f"| {'horizon_num':<12} = {self.horizon_num:>6} |")
        print(f"| {'vertical_num':<12} = {self.vertical_num:>6} |")
        print(f"| {'traj_num':<12} = {self.traj_num:>6} |")
        print("-------------------------")
...
class LatticePrimitive(LatticeParam):
    ...
    def get_lattice_yaw_for_segment(self, segment_idx):
        num_segments = cfg["lidar_lattice_segments"]
        segment_yaw = -torch.pi + (2 * torch.pi / num_segments) * segment_idx + (torch.pi / num_segments)
        return segment_yaw
